...
/**
 * 継続(改ページ運搬)の特性テストです(P4: LegacyOpenTail 縮小の
...
public class ContinuationCharacterizationTest extends TestCase {
	...
	public void testTableSpanningBreakChainsToo() throws Exception {
		...
		// LegacyOpenTail(開きテキスト・moved-open の従来規約)になる。
		// 収集不能の全 restyle(LEGACY_ROOTS)はゼロ — この現状を保存契約
		// として固定する(P4 は LEGACY_TAILS の縮小が対象)
		...
		assertEquals("収集不能の Legacy 全 restyle が発火", 0, ContinuationStats.LEGACY_ROOTS.get());
	}
