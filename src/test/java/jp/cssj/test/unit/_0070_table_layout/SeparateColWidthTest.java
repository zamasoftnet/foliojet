...
public class SeparateColWidthTest extends AbstractTestCase {
	protected void transcode() throws Exception {
		File file = new File(
				"files/unittest/0070-table-layout/separate-col-width.html");
		CTISessionHelper.transcodeFile(this.session, file, "text/html", null);
	}

	public SeparateColWidthTest(String name) {
		super(name);
	}
	...
	public boolean check_a(IBox box, int pageNumber, double x, double y) {
		if (box.getType() == BoxType.TABLE_CELL) {
			...
			assertEquals(116, box.getWidth(), 1);
			return true;
		}
		return false;
	}
	...
	public boolean check_d(IBox box, int pageNumber, double x, double y) {
		if (box.getType() == BoxType.TABLE_CELL) {
			System.out.println(box.getWidth());
			assertEquals(76, box.getWidth(), 1);
			return true;
		}
		return false;
	}
