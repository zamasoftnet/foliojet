 ...
 * (ChatGPT Pro相談で指摘)。{@link #expectedOps}が{@link ResumeProgram}
...
public sealed interface ResumeOp {
	...
	/**
	 * {@link ResumeProgram}から、既存{@code resumeFrame()}実行時に期待
	 * される操作列を機械的に導出します。副作用なし(fragment生成・
	 * builder状態変異は一切行わない)。
	 */
	static List<ResumeOp> expectedOps(final ResumeProgram program) {
